...
app.get("/calculate", (req, res) => {
    try {
        ...
        if (isNaN(parsedN1) || isNaN(parsedN2)) {
            logger.error("Invalid parameters provided"); // Log error if parameters are invalid
            throw new Error("Invalid parameters provided"); // Throw an error
        }

        // Check if requested operation is supported
        if (!operations[operation]) {
            logger.error("Invalid operation requested"); // Log error if operation is invalid
            throw new Error("Invalid operation requested"); // Throw an error
        }
        ...
        logger.error(error.toString()); // Log the error message
        res.status(500).json({ statusCode: 500, msg: error.toString() }); // Send error response
    }
});
